<?php
    ...
    {
        $mySQLStr = "SELECT * FROM \"wards_from_2012\"";
    ...
    {
        $mySQLStr = "SELECT ST_AsGeoJson(geom) as geo from \"wards_from_2012\"";
        $result = query($paPDO, $mySQLStr);
    ...
    function getInfoCMRToAjax($paPDO,$paSRID,$paPoint)
    {
        ...
        $mySQLStr = "SELECT rep_name,rep_phone, pop_male, pop_female,  areasqmi, pop_2011_2 from \"wards_from_2012\" where ST_Within('SRID=".$paSRID.";".$paPoint."'::geometry,geom)";
        //echo $mySQLStr;
        //echo "<br><br>";
        $result = query($paPDO, $mySQLStr);
        ...
        if ($result != null)
        {
            ...
            foreach ($result as $item){
                ...
                $resFin = $resFin.'<tr><td><b>Diện tích</b> '.$item['areasqmi'].' Mét vuông</td></tr>';
                $resFin = $resFin.'<tr><td><b>Dân số:</b> '.$item['pop_2011_2'].'</td></tr>';
                $resFin = $resFin.'<tr><td><b>Số điện thoại:'.$item['rep_phone'].'</b></td></tr>';
                $resFin = $resFin.'<tr><td><b>Nam giới:</b> '.$item['pop_male'].'</td></tr>';
                $resFin = $resFin.'<tr><td><b>Nữ giới:</b> '.$item['pop_female'].'</td></tr>';
                break;
            }
            $resFin = $resFin.'</table>';
            return $resFin;
